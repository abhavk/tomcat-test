VolpayApp.controller('mpidetailsCtrl', function ($scope, $http, $state, $location, $stateParams, $timeout, GlobalService, LogoutService, $filter,getMethodService) {
	...
	$scope.SaveTemplate = !GlobalService.fromAddNew;
	...
	console.log("fromAddNew", GlobalService.fromAddNew, "viewclicked", GlobalService.ViewClicked)

	$scope.TemplateDetails = $scope.FXRate;
	$scope.TemplateDetails.Status = "ACTIVE";
	...
	var remoteDataConfig11 = function () {
		
		
		$http({
				url : BASEURL + '/rest/v2/parties/readall',
				method : "POST",
				data : {"start":0,"count":1000},
				headers : {
					'Content-Type' : 'application/json'
				}
	...
	$scope.getPaymentTypeDetails = function (ProductsSupported, Service) {
		...
		if(GlobalService.fromAddNew==false){
	...
	$scope.deleteData = function () {
		...
		$scope.delObj.TemplateName = $scope.FXRate.TemplateName
			...
			$http.post(BASEURL + RESTCALL.MPITemplates + 'delete', $scope.delObj).success(function (data) {
