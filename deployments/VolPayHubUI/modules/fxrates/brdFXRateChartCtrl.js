VolpayApp.controller('brdFXRateChartCtrl', function ($scope, $state, bankData, GlobalService, $timeout,$location,$http,LogoutService) {
...
$scope.Status = GlobalService.storeStatus ? GlobalService.storeStatus : $scope.status1;
	...
	var restServer = RESTCALL.BankFXRateChartREST + 'readall';
            	var delData = {};
            	$scope.backUp = {};
            	$scope.indexx = "";
			...
			//console.log($scope.bankData)
            	...
            	$scope.deleteData = function() {
            		...
            		restServer = RESTCALL.BankFXRateChartREST+'delete';
            	...
            	function getData(response) {
                   ...
                   // $scope.bankData.QueryOrder = [{"ColumnName":$scope.orderByField,"ColumnOrder":$scope.SortType}];
                    ...
                    restServer = RESTCALL.BankFXRateChartREST+'readall';
            		bankData.crudRequest("POST", restServer,$scope.bankData).then(applyRestData,errorFunc);
            	}
 ...
 $scope.multiSortObj=[]
	...
	/** List and Grid view Ends**/
		...
		crudRequest("POST",RESTCALL.BankFXRateChartREST+"readall",{"start": argu,"count": ($scope.TotalCount > 1000) ? 1000 : $scope.TotalCount}).then(function(response){	
