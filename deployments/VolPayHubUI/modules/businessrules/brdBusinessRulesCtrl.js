VolpayApp.controller('brdBusinessRulesCtrl', function ($scope, $state, $location, $http, bankData, GlobalService, $timeout,LogoutService,loadmeOnscroll) {

$scope.status1 = [{"actualvalue":"ACTIVE","displayvalue":"ACTIVE"},{"actualvalue":"SUSPENDED","displayvalue":"SUSPENDED"},{"actualvalue":"CREATED","displayvalue":"CREATED"},{"actualvalue":"WAITINGFORAPPROVAL","displayvalue":"WAITINGFORAPPROVAL"},{"actualvalue":"APPROVED","displayvalue":"APPROVED"},{"actualvalue":"FORREVISION","displayvalue":"FORREVISION"},{"actualvalue":"REJECTED","displayvalue":"REJECTED"},{"actualvalue":"DELETED","displayvalue":"DELETED"}];
...
$scope.Status = GlobalService.storeStatus ? GlobalService.storeStatus : $scope.status1;
 ...
 $scope.restResponse = {};
	...
	if ((sessionStorage.newEditRule == true) || (sessionStorage.newEditRule == 'true')) {
		...
		// alert(sessionStorage.newEditRule)
		//alert()

		//$scope.creatingWindow=false;
		//console.log(sessionStorage.newRuleFormData);
		//console.log(sessionStorage.buildedRule)
...
$scope.flowchart = function(val,$event){
	...
	$scope.deleteData = function () {
		...
		var deleteObj = {"OfficeCode": delData.OfficeCode, "RuleCode": delData.RuleCode}
			...
			for(k in $scope.bankData.sorts){
				if($scope.bankData.sorts[k].columnName == dat.FieldName){
					if($scope.bankData.sorts[k].sortOrder == 'Asc'){
						...
						orderFlag = false;
						$('#'+dat.FieldName+'_icon').attr('class','fa fa fa-minus fa-sm')
						$('#'+dat.FieldName+'_Icon').removeAttr('class')
                        break;
					}
				}
...
$scope.buildFilter = function(argu1,argu2){	
    ...
    $scope.setEffectivedate = function(val,to){
    		...
    		if($scope.selectedDate == val.displayvalue){
    			...
    			$('.filterBydate').css({'background-color':'#fff','box-shadow': '1.18px 2px 1px 1px rgba(0,0,0,0.40)'})
    			$('#'+$scope.selectedDate.replace(/\s+/g, '')).css({'box-shadow':'','background-color':'#d8d5d5'})
    		}
	          ...
	          $scope.viewData = function(data,flag)
               {
                ...
                $state.go('app.businessrulesdetails',{input:$scope.permission})
            ...
            function($)
                {
                    ...
                    {
                        $scope.widthOnScroll();
                        //console.log("len in table" , $scope.lenthofData)
                    ...
                    //         $scope.widthOnScroll();
                    //         //console.log($scope.lenthofData.length, $(window).scrollTop() + $(window).height())
       ...
       /* $("#expbtn").click(function(){
            ...
            if(radioValue == 'All'){
                alert("Your are a - " + radioValue);
                 crudRequest("POST",restServer,{}).then(function(response){
                      
                      $scope.alldata = response.data.data;
                      console.log($scope.alldata);
											$scope.exportAsExcel($scope.alldata);

                 })
            }
            else{
            		$scope.exportAsExcel($scope.restVal);
            }
        });*/
        

	/*$scope.exportAsExcel = function(data){
		$scope.dat = angular.copy(data);
		console.log($scope.dat);
		$scope.dat.shift();
		//bankData.exportToExcel($scope.dat, $scope.Title)
		JSONToCSVConvertor(bankData, $scope.dat, 'BusinessRules', true);
	}*/
