...
function dBoardtoFileList(GlobalService, status, PSACode) {
    GlobalService.orderByField = 'EntryDate';
    GlobalService.sortReverse = false;
    GlobalService.sortType = 'Desc';
    GlobalService.isSortingClicked = false;
   ...
   //console.trace(arg)
     ...
     //   var cntAmt;
        ...
        if (arg.id == 'InboundModal') {
            $('#InboundTotalforModal').html($('#resizeWindow').find('.caption').find('span').text() + " - " + name + ' (' + cenVal + ')')
    ...
    centerTextforDonut: function (arg) {
        ...
        return function () {
            ...
            { cenTxt = 'Total Count'; }
            else if (arg.selectVal == 'Amount')
            { cenTxt = 'Total Inbound'; }
    ...
    ChartToPaymentsNav: function (arg) {
         ...
         else if ((arg.chartId == 'CurDis') || (arg.chartId == 'sankeyChart')||(arg.chartId == 'CurDisCustom') || (arg.chartId == 'sankeyChartCustom')) {
        ...
        nv.addGraph(function () {
            nv.addGraph(function () {
                var Inboundchart = nv.models.pieChart()
                    ...
                    .showLegend(false).showLabels(true).donut(true).donutRatio(0.48).color(d3.scale.myColors().range()).labelType('percent').labelThreshold(.035).labelSunbeamLayout(true).margin({ top: 0, bottom: 0, left: 0, right: 0 })
                    .tooltipContent(function (key, y, e, graph) {

                      //  console.log(arg,arg.id)
                      //console.log("donut",$('#'+arg.id).parents()[1])
                     console.log("donut",$('#'+arg.id).parent().parent().parent().find('.portlet-title').find('.actions').find('.custDropDown button span').text())

                     var newDonutCntAmt = $('#'+arg.id).parent().parent().parent().find('.portlet-title').find('.actions').find('.custDropDown button span').text();
                        ...
                        var container = '<div style="padding:7px 4px;"><div style="float:left;"><div class="tooltipBox" style="margin-top:7px;background-color:' + key.color + '"></div></div><div style="float:left;margin-left:5px;margin-top:3.5px;">' + key.data.Name + " (" + key.data.Currency + ")" + '</div><br/><div class="txtCenter" style="float:left;width:100%;margin-top:3.5px;margin-bottom:7px;">' + currencyVal + '</div></div>'
                        return container;
            ...
            nv.addGraph(function () {
                ...
                var Inboundchart = nv.models.pieChart()
                    ...
                    .tooltipContent(function (key, y, e, graph) {
                       ...
                       console.log("pie", $('#'+arg.id).parent().parent().parent().parent().find('.portlet-title').find('.actions').find('.custDropDown button span').text())
                          var newCntAmt = $('#'+arg.id).parent().parent().parent().find('.portlet-title').find('.actions').find('.custDropDown button span').text()
      ...
      //  console.log(arg)
        ...
        for (var i in genObj) {
            ...
            if ((arg.id == 'CurDis') || (arg.id == 'sankeyChart') || (arg.id == 'CurDisCustom') || (arg.id == 'sankeyChartCustom') || ((arg.id == 'InboundModal') && (arg.basedOn == 'PaymentStatus'))) {
             ...
             //console.log(genObj[i],myColors[i],genObj[i].key)
        }
        ...
        //     if (($scope.inbndModalDefChart == 'vertical')) {
        //         d3.select('#' + arg.id).select('.nv-barsWrap').select('.nv-groups').selectAll('.nv-group').selectAll('text').remove();
        //         d3.select('#' + arg.id).select('.nv-barsWrap').select('.nv-groups').selectAll('#TotalVal').remove();
        //         MultiChart.update();
        ...
        //         $('.inbndModalBody').css({ 'width': wid - 32 + "px", 'height': hgt - 120 + "px" })
        //         $('#InboundModal').css({ 'width': wid + "px", 'height': hgt - 150 + "px" })
        //         $('.modaltestBlock').height($('#InboundModal').height())
        //     }
    ...
    HorizontalMultibarChart: function (arg) {
        ...
        nv.addGraph(function () {
            ...
            $('#resizeWindow').bind('resize', function (e) {
                ...
                if ($scope.inbndModalDefChart == 'horizontal') {
                    setTimeout(function () {
                        chart2showLabels()
                    }, 500)
                    chart2.update();
